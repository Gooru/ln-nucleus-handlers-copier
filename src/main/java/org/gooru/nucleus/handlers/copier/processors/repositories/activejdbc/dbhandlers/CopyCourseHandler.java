...
class CopyCourseHandler implements DBHandler {
  ...
  public ExecutionResult<MessageResponse> validateRequest() {
    ...
    LazyList<AJEntityCourse> courses = AJEntityCourse.where(AJEntityCourse.AUTHORIZER_QUERY, this.context.courseId(), false);

    // Course should be present in DB
    if (courses.size() < 1) {
      LOGGER.warn("Course id: {} not present in DB", context.courseId());
      return new ExecutionResult<>(MessageResponseFactory.createNotFoundResponse(MESSAGES.getString(MessageCodeConstants.CP015)),
          ExecutionResult.ExecutionStatus.FAILED);
    }
  ...
  public ExecutionResult<MessageResponse> executeRequest() {
    ...
    int count =
        Base.exec(AJEntityCourse.COPY_COURSE, copyCourseId, userId, userId, userId, courseId, courseId, "[\"" + userId + "\"]", userId, courseId);
    if (count > 0) {
      ...
      if (unitCount > 0) {
        ...
        if (lessonCount > 0) {
          int collectionCount = Base.exec(AJEntityCourse.COPY_COLLECTION, userId, userId, userId, copyCourseId, courseId);
      ...
      return new ExecutionResult<>(MessageResponseFactory.createCreatedResponse(copyCourseId.toString(),
          EventBuilderFactory.getCopyCourseEventBuilder(copyCourseId.toString())), ExecutionResult.ExecutionStatus.SUCCESSFUL);
    }
